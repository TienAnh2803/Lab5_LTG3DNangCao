...
namespace CartoonFX
{
	public partial class CFXR_Effect : MonoBehaviour
	{
		...
		public class CameraShake
		{
			...
			public Vector3 shakeStrength = new Vector3(10f, 10f, 10f);
			...
			public void animate(float time)
			{
				...
				if (time < totalDuration)
				{
					...
					// Lắc cực mạnh không giới hạn
					var randomVec = new Vector3(Random.Range(-10f, 10f), Random.Range(-10f, 10f), Random.Range(-10f, 10f));
					shakeVector = Vector3.Scale(randomVec, shakeStrength) * shakeCurve.Evaluate(delta) * GLOBAL_CAMERA_SHAKE_MULTIPLIER;
				}
				else if (isShaking)
				{
					StopShake();
